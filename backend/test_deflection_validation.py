...
def test_3_column_supported_4corners():
    ...
    # Check column nodes undergo elastic axial shortening
    Kz_col = E_col * (col_w * col_d) / col_H
    w_expected_shortening = (q_Nm2 * a**2 / 4.0) / Kz_col
    for nid in col_node_ids:
        for d in result.nodeDeflections:
            if d.nodeId == nid:
                print(f"  Column node {nid}: wz = {abs(d.wz)*1000:.6f} mm (expected max elastic shortening: {w_expected_shortening*1000:.6f} mm)")
                assert 0.001 / 1000.0 <= abs(d.wz) <= w_expected_shortening * 1.05, f"Column node {nid} elastic shortening out of tolerance: wz={d.wz}"
